...
int main(int argc, char *argv[])
{
    ...
    if (!opt_suppressOutput)
    {
        ...
        /* Auto apply VOI window if exists */
        auto windowCnt = di->getWindowCount();
        if(windowCnt > 0 and (opt_windowParameter < 1 or opt_windowParameter > windowCnt))
        {
            opt_windowType = 1;
            opt_windowParameter = 1; // the first VOI window
        }else{
            DVPresentationState *pstate = new DVPresentationState();
            if(pstate->createFromImage(*(dfile->getDataset())).good() and pstate->haveActiveVOILUT() == OFFalse)
            {

                if(pstate->haveActiveVOIWindow() == OFTrue)
                {
                    pstate->getCurrentWindowCenter(opt_windowCenter);
                    pstate->getCurrentWindowWidth(opt_windowWidth);
                }else{
                    double minVal, maxVal;
                    pstate->getImageMinMaxPixelValue(minVal,maxVal);
                    opt_windowWidth = maxVal - minVal;
                    opt_windowCenter = minVal + (maxVal - minVal + 1) / 2;
                }
                opt_windowType = 5; // set center and width
            }
            delete pstate;
        }
        ...
        switch (opt_windowType)
        {
            case 1: /* use the n-th VOI window from the image file */
                if ((opt_windowParameter < 1) || (opt_windowParameter > windowCnt))
                {
                    OFLOG_FATAL(dcm2pnmLogger, "cannot select VOI window " << opt_windowParameter << ", only "
                        << windowCnt << " window(s) in file");
                    return 1;
                }
